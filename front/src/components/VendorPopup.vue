  ...
  <q-dialog persistent ref="dialogRef" @hide="onDialogHide" :backdrop-filter="'blur(4px)'">
    <q-card class="q-dialog-plugin" style="width: 700px; max-width: 80vw;">
      <q-card-section class="row items-center mx-2 border-b-2">
        <div class="text-h6">{{ mode }} Vendor</div>
        <q-space />
      ...
      <q-form @submit.prevent="onSubmit(row)">
        ...
        <q-card-section class="row items-center">
          <q-input class="w-full" label="Address" v-model="row.address"/>
        </q-card-section>
        <q-card-section class="row items-center">
          <q-input class="w-full" label="URL" v-model="row.url"/>
        </q-card-section>
