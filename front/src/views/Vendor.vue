<template>
  <div class="q-pa-xs">    
    ...
    <template v-slot:top="props">
        <div class="col-2 q-table__title">Vendor</div>
...
import VendorPopup from '../components/VendorPopup.vue';
...
export default {
  setup() {
    ...
    // get vendor data on mounted
    onMounted(async () => {
      try {
        $q.loading.show();
        const { data: { data } } = await get('/vendors');
        rows.push(...data);
      } catch (err) {
        $q.notify({
